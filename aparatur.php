...
<header>
  ...
  <!-- ================= Konten Aparatur ================= -->
<section class="aparatur-section">

  <!-- KIRI: daftar aparatur -->
  <div class="aparatur-kiri">
    <h2 class="aparatur-judul">Struktur Aparatur Desa Baturusa</h2>
    <div class="aparatur-list">
      <?php
      $data_aparatur = mysqli_query($koneksi, "SELECT * FROM aparatur ORDER BY id ASC");
      if (mysqli_num_rows($data_aparatur) > 0):
        while ($a = mysqli_fetch_assoc($data_aparatur)):
      ?>
        <div class="aparatur-item">
          <img src="admin/assets/images/aparatur/<?php echo htmlspecialchars($a['foto']); ?>" 
               alt="<?php echo htmlspecialchars($a['nama']); ?>">
          <div class="aparatur-info">
            <h3><?php echo htmlspecialchars($a['nama']); ?></h3>
            <p><strong>Jabatan:</strong> <?php echo htmlspecialchars($a['jabatan']); ?></p>
            <p><strong>Tempat, Tanggal Lahir:</strong> <?php echo htmlspecialchars($a['tempat_tanggal_lahir']); ?></p>
            <p><strong>Masa Jabatan:</strong> <?php echo htmlspecialchars($a['masa_jabatan']); ?></p>
          </div>
        </div>
      <?php endwhile; else: ?>
        <p class="text-center">Belum ada data aparatur.</p>
      <?php endif; ?>
    </div>
  </div>
