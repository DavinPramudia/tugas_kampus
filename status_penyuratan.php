...
// ================= KONEKSI DATABASE =================
session_start();
include 'admin/koneksi.php'; // pastikan path benar

if (!isset($_GET['id'])) {
    echo "<script>alert('ID surat tidak ditemukan!'); window.location='surat.php';</script>";
    exit;
}
...
if (!$data) {
    echo "<script>alert('Data Surat tidak ditemukan!'); window.location='surat.php';</script>";
    exit;
}
...
<style>
/* Timeline CSS sama persis dengan yang sebelumnya */
.timeline-container {
    background: white;
    border-radius: 14px;
    padding: 30px;
  ...
  <div class="page-title" style="background-image: url('./image/page_title_3.jpg');">
    <div class="container">
        ...
        <nav class="breadcrumbs">
            <ol>
                <li><a href="index.php">Beranda</a></li>
                <li class="current">Status Surat</li>
            </ol>
        </nav>
    </div>
</div>
...
<section class="berita-section">
    ...
    <div class="berita-kiri">
        <article class="status-laporan">
            <h2 class="pengaduan-judul text-center">📊 Status Surat Anda</h2>
            <p class="pengaduan-deskripsi text-center">
                Lacak progres surat yang telah Anda ajukan ke Pemerintah Desa Baturusa.
            </p>
            ...
            <div class="timeline-container">
                <div class="timeline">
                    ...
                    <div class="timeline-step <?= ($status == 'selesai') ? 'done' : '' ?>">
                        ...
                        <p>
                            Selesai
                            <?php if ($status == 'selesai' && (!empty($data['hasil_file']))) : ?>
                                <br>
                                  <a href="hasil_penyuratan.php?id=<?= $data['id']; ?>" class="lihat-hasil-link">📄 Lihat Hasilnya</a>
                            <?php endif; ?>
                        </p>
                    </div>
                </div>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (status === "ditolak") {
        Swal.fire({
            icon: "error",
            title: "Surat Ditolak",
            html: `<p>Alasan: <strong>${alasan || 'Tidak ada alasan'}</strong></p><p>Ingin membuat surat baru?</p>`,
            ...
            if (result.isConfirmed) {
                window.location.href = "penyuratan.php";
            } else {
    ...
    if (status === "selesai") {
        Swal.fire({
            icon: "success",
            title: "Surat Selesai 🎉",
            text: "Terima kasih! Surat Anda telah selesai dibuat. Ingin membuat surat lain?",
            showCancelButton: true,
