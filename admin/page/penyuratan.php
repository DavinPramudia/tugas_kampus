...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    elseif ($action == 'hapus') {
        ...
        echo $hapus
            ? "<script>alert('Penyuratan berhasil dihapus'); window.location='index.php?page=penyuratan';</script>"
            : "<script>alert('Gagal menghapus surat');</script>";
        exit;
    }
}

// ==== AMBIL SEMUA DATA PENYURATAN ====
        ...
        <table class="table table-bordered table-striped align-middle user-table">
          ...
          <tbody>
              ...
              <td class="text-center">
                ...
                <?php if ($status == 'Menunggu Verifikasi'): ?>
                  <div class="mt-2 user-btn-group btn-group">
                    <a href="index.php?page=penyuratan&action=Diterima&id=<?= $row['id'] ?>" class="btn btn-sm btn-success">✅ Terima</a>
            ...
            <?php if(mysqli_num_rows($query)==0): ?>
              <tr><td colspan="9" class="text-center text-muted">Belum ada data surat.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
...
function downloadPDF(elementId, modalId) {
    ...
    myWindow.document.write('<html><head><title>Surat</title></head><body>');
...
function downloadPDF(elementId) {
  var content = document.getElementById(elementId).innerHTML;
  var myWindow = window.open('', '', 'width=800,height=600');
  myWindow.document.write('<html><head><title>Surat PDF</title>');
