...
// PAGINATION SETUP
// ==========================
$limit = 1; // jumlah berita per halaman
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$start = ($page - 1) * $limit;

// ambil berita sesuai halaman
$query = mysqli_query($koneksi, "SELECT * FROM berita ORDER BY id DESC LIMIT $start, $limit");

// hitung total berita
$total_result = mysqli_query($koneksi, "SELECT COUNT(*) AS total FROM berita");
  ...
  <div class="berita-kiri">
    <div class="berita-grid">
      <?php while ($row = mysqli_fetch_assoc($query)): ?>
        <div class="card">
          <img src="admin/assets/images/berita/<?php echo htmlspecialchars($row['gambar']); ?>" alt="Gambar Berita">
          <div class="card-body">
            <h5 class="card-title"><?php echo htmlspecialchars($row['judul']); ?></h5>
            <p class="card-text">
              <?php echo substr(strip_tags($row['isi']), 0, 100); ?>...
            </p>
            <a href="berita_detail.php?id=<?php echo $row['id']; ?>" class="btn-berita">Baca Selengkapnya</a>
          </div>
        </div>
      <?php endwhile; ?>
    </div>
    ...
    <div class="pagination-container">
      <div class="pagination">
        <?php if ($page > 1): ?>
          <a href="berita.php?page=<?php echo $page - 1; ?>">&laquo;</a>
        <?php endif; ?>

        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
          <a href="berita.php?page=<?php echo $i; ?>" class="<?php echo ($i == $page) ? 'active' : ''; ?>">
            <?php echo $i; ?>
          </a>
        <?php endfor; ?>

        <?php if ($page < $total_pages): ?>
          <a href="berita.php?page=<?php echo $page + 1; ?>">&raquo;</a>
        <?php endif; ?>
      </div>
    </div>
  </div>
