...
if (mysqli_num_rows($query) == 0) {
  echo "<h2 style='text-align:center;'>umkm tidak ditemukan.</h2>";
  exit();
}

$umkm = mysqli_fetch_assoc($query);
?>
...
<head>
  ...
  <title><?php echo htmlspecialchars($umkm['judul']); ?> - umkm Desa Baturusa</title>
...
<!-- ================= Page Title ================= -->
  ...
  <div class="container">
    <h1><?php echo htmlspecialchars($umkm['judul']); ?></h1>
    <nav class="breadcrumbs">
      <ol>
        <li><a href="index.html">Beranda</a></li>
        <li><a href="umkm.php">UMKM</a></li>
        <li class="current"><?php echo htmlspecialchars($umkm['judul']); ?></li>
      </ol>
    </nav>
  </div>
</div>
...
<section class="detail-berita-section">
  <div class="detail-container">
    ...
    <!-- KIRI: artikel utama -->
    <article class="detail-berita-card">
      <h2><?php echo htmlspecialchars($umkm['judul']); ?></h2>
      <small class="tanggal">
        Dipublikasikan pada: <?php echo date('d F Y', strtotime($umkm['tanggal'] ?? 'now')); ?>
      </small>

      <?php if (!empty($umkm['gambar']) && file_exists("admin/assets/images/umkm/" . $umkm['gambar'])): ?>
        <div class="umkm-gambar-wrapper">
          <img src="admin/assets/images/umkm/<?php echo htmlspecialchars($umkm['gambar']); ?>" 
              alt="<?php echo htmlspecialchars($umkm['judul']); ?>" 
              class="umkm-gambar">
      </div>
      ...
      <div class="isi-berita">
        <?php echo nl2br($umkm['isi']); ?>
      </div>

      <div class="kembali-btn">
        <a href="umkm.php" class="btn-berita">&laquo; Kembali ke Daftar UMKM</a>
      </div>
    </article>
